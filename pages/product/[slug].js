...
function SLug(props) {
  ...
  async function addtoCart(param) {
    console.log(iqty <= 0);
    if (iqty <= 0) {
      alert("Product add to the card")
      setIqty(iqty+1)
      setQty(qty+1)
      const data = await axios.post(`${window.origin}/api/addproduct`,
                                  {
                                    title:param.title,
                                    price:param.price,
                                    image:param.image,
  ...
  function move(e) {
    ...
    if (x > image.width - (w / 3)) {x = image.width - (w / 3);}
    if (x < w / 3) {x = w / 3;}
    if (y > image.height - (h / 3)) {y = image.height - (h / 3);}
    if (y < h / 3) {y = h / 3;}
    mag.style.left = (x - w) + "px";
    mag.style.top = (y - h) + "px";
    mag.style.backgroundPosition = "-" + ((x * 3) - w + bw) + "px -" + ((y * 3) - h + bw) + "px";  
        ...
        <div className={styles.maintainBox}>
          <div className={styles.maincontainer}>
            <div className={styles.containerbox}>
                ...
                <Image id='img' priority={true} onMouseMove={(e)=>{startMove(e)}} src={pimg ? pimg : data['image'] } alt={data.title} width={100} height={100} layout={'responsive'}></Image>
                <div id='mag' onMouseLeave={(e)=>{leave(e)}} onMouseMove={(e)=>{move(e)}} className={styles.mag}></div>
                  ...
                  {img.map((pro_img,key)=>{
                    return(
                      <div key={key} className={styles.imgslide}>
                        <Image src={pro_img.image} key={key} priority={true} onClick={()=>{setImage(pro_img.image)}} alt={data.title} width={100} height={100} layout={'responsive'}></Image>
                      </div>
                    )
                  })}
                </div>
            ...
            <div className={styles.mainbox}>
              <p>Product/{data['title']}</p><br/>
              <h1 className={styles.title}>{data['title']}</h1><br/>
              <h2 className={styles.descripsion}>Price:${data['price']}</h2>
              
              <div className={styles.sizebox}>
              <button onClick={()=>{increaseQty()}} className={styles.sizebtn}><h2>M</h2></button>
              <button onClick={()=>{increaseQty()}} className={styles.sizebtn}><h2>L</h2></button>
              <button onClick={()=>{increaseQty()}} className={styles.sizebtn}><h2>XL</h2></button>
              </div>
                
              <div className={styles.allbox}>
                <div className={styles.qtybox}>
                  <button onClick={()=>{increaseQty(data)}} className={styles.btn}><GoPlus/></button>
                  <p>{iqty}</p> 
                  <button onClick={()=>{decreaseQty(data)}} className={styles.btn}><FaMinus/></button>
                </div>
                <div className={styles.qtybox}>
                  <button onClick={()=>{addtoCart(data)}} className={styles.cartbtn}>Add to Cart</button> 
                  <button onClick={()=>{buyNow()}} className={styles.cartbtn}>Buy Now</button>
                </div>
              </div>
